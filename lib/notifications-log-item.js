...
module.exports = (NotificationsLogItem = (function () {
	NotificationsLogItem = class NotificationsLogItem {
		...
		render() {
			...
			notificationView.onChange(() => this.onNotificationChange(notificationView));
		...
		renderNotification(view) {
			...
			if (nButtons != null) {
				for (const button of Array.from(nButtons.children)) {
					...
					for (const tooltip of Array.from(atom.tooltips.findTooltips(button))) {
		...
		onNotificationChange(view) {
			...
			if (notificationElement) {
				return this.element.replaceChild(this.renderNotification(view), notificationElement);
			}
		}
