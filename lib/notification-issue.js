		...
		return fetch(`https://api.github.com/search/issues?q=${encodeURIComponent(query)}&sort=created`, {headers: githubHeaders})
			...
			.then(function (data) {
				if ((data != null ? data.items : undefined) != null) {
					const issues = {};
					for (const issue of Array.from(data.items)) {
	...
	getIssueBody() {
		return new Promise((resolve, reject) => {
			...
			return Promise.all([systemPromise, nonCorePackagesPromise]).then(all => {
				...
				if (packageName != null) {
					packageVersion = __guard__(__guard__(atom.packages.getLoadedPackage(packageName), x1 => x1.metadata), x => x.version); 
				}
				const copyText = "";
...
`;
	...
	getPackagePathsByPackageName() {
		...
		for (const pack of Array.from(atom.packages.getLoadedPackages())) {
