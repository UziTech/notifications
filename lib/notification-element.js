...
(NotificationElement = (function () {
	NotificationElement = class NotificationElement {
		...
		constructor(model, visibilityDuration) {
			...
			this.visibilityDuration = (left = this.model.getOptions().timeout) != null ? left : this.visibilityDuration;
...
`;
          ...
          ((checkFatalIssues === "telemetry") && (telemetryConsent !== "no"));
				...
				issueButton.addEventListener("click", e => {
					e.preventDefault();
					return this.renderPromise = checkForIssues();
				}
				, {once: true});
				return Promise.resolve();
		...
		makeDismissable() {
			if (!this.model.isDismissable()) {
				...
				return this.element.removeEventListener("mouseleave", this.handleElementMouseLeave);
			}
		}
		...
		handleElementClick() {
			return this.makeDismissable();
		}

		handleElementMouseEnter() {
			return this.element.classList.add("mouse-over");
		}

		handleElementMouseLeave() {
			return this.element.classList.remove("mouse-over");
		}
...
var addSplitLinesToContainer = function (container, content) {
	...
	for (const line of Array.from(content.split("\n"))) {
