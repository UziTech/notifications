...
import NotificationIssue from "./notification-issue";
...
export default class NotificationElement {
	constructor(model, visibilityDuration) {
		...
		this.visibilityDuration = this.model.getOptions().timeout || visibilityDuration;
...
`;
	...
	removeNotificationAfterTimeout() {
		...
		setTimeout(() => {
			this.element.remove();
		}, NotificationElement.animationDuration);
	}
