...
class CommandLogger {
	static instance() {
		return this._instance ? this._instance : (this._instance = new CommandLogger());
	}
	...
	logCommand(command) {
		...
		if (command.detail && command.detail.jQueryTrigger) {
			return;
		}
	...
	// ```
	eachEvent(fn) {
		for (let offset = 1; offset <= this.logSize; offset++) {
	...
	formatEvent(event, lastTime) {
		...
		let classText = "";
		if (targetClassName) {
			for (const klass of targetClassName.split(" ")) {
				classText += `.${klass}`;
			}
		}
