...
export default class StatusBarManager {
	constructor(statusBar, initialNotifications) {
		this.state = {
			count: initialNotifications.length,
			lastType: initialNotifications.length > 0
				? initialNotifications[initialNotifications.length - 1].getType()
				: "",
			newNotification: initialNotifications.length > 0,
		};
