...
(StatusBarManager = (function() {
  StatusBarManager = class StatusBarManager {
    ...
    render() {
      ...
      return (() => {
        const result = [];
        for (let notification of Array.from(atom.notifications.getNotifications())) {
    ...
    onClick() { return atom.commands.dispatch(this.element, 'notifications-plus:toggle-log'); }

    onAnimationEnd(e) { if (e.animationName === 'new-notification') { return this.number.classList.remove('new-notification'); } }
